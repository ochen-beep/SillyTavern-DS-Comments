...
Направление фикса под требование «выбор постоянен до ручной смены»: перенести тексты
шаблонов в `extensionSettings` (как jailbreakText — серверно и кроссбраузерно, разовая
миграция из localforage) + убрать персист из обоих фолбэков (выбор меняется только
явным действием пользователя; при отсутствии текста — видимая ошибка, транзиентный
'main' без записи в настройки).
