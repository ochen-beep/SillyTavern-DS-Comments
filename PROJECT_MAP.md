...
> Реализовано 2026-09-16: **soft-stale стал видимым** — `updatePostIndicator` вешает `dsc_stale` (янтарный + пунктир, тултип «перегенерируйте») когда restore вернул `soft-stale`; статус хранится в `cache._lastRestoreStale`, сбрасывается storeFeed/clean-hit/missing; `selectCommentaryTarget` сам обновляет индикатор после fp-вердикта (кроме superseded). **Дебаг-функции** (Debug Menu ST) выводят результат в console + toastr-превью — ST отбрасывает возвращаемое значение (power-user.js: `functionRecord.func()`), обёртка в `registration-lifecycle.js`. **Обсервер**: после исчерпания rAF-ретраев (zero-height #chat) — до 4 медленных ретраев по 2.5 с вместо окончательной сдачи; сброс на успешный attach и в disconnectObservers. **generator.buildPastCommentParts** не выпускает пустые блоки (тред из одних oversized-комментариев пропускается, фрейминг «previous chapter» остаётся привязан к позиции треда).
