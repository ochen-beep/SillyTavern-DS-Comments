...
> Проверено: `npm test` — **586 pass / 0 fail** (node --test, ~0.8 с, NODE_TEST=1).
...
> Реализовано 2026-09-16: **soft-stale остаётся внутренним сигналом** — restore-вердикт пишется в restore/event log, но UI сознательно НЕ помечает «устаревшие» фиды: правки ролевых постов бывают намеренными, дрейф fp — норма (попытка `dsc_stale`-метки в индикаторе отменена по решению владельца). **Фикс CHAT_CHANGED**: saveMode-ветка завершалась ранним `return` ДО `scheduleObserverRestart('chat', 600)` — после смены чата обсервер оставался привязан к узлам СТАРОГО чата; теперь рестарт планируется и в этой ветке (проверено логом: `observer: init/bound` после смены чата появился). **Дебаг-функции** (Debug Menu ST) выводят результат в console + toastr-превью — ST отбрасывает возвращаемое значение (power-user.js: `functionRecord.func()`), обёртка в `registration-lifecycle.js` (именно это позволило продиагностировать observe-баги вживую). **Обсервер**: после исчерпания rAF-ретраев (zero-height #chat) — до 4 медленных ретраев по 2.5 с вместо окончательной сдачи; сброс на успешный attach и в disconnectObservers. **generator.buildPastCommentParts** не выпускает пустые блоки (тред из одних oversized-комментариев пропускается, фрейминг «previous chapter» остаётся привязан к позиции треда). ⚠ Живая отладка scroll-follow в IAB-хосте с ручным продвижением кадров (rAF/scroll-события заморожены между скриншотами) невозможна — «мёртвый» фоллоу в таких сессиях = артефакт стенда, не баг.
