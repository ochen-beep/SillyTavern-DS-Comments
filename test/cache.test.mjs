...
test('soft-stale restore marks the indicator stale; a fresh store clears it', async () => {
    const ctx = getCtx();
    ctx.chat = [null, null, { mes: 'post', is_user: false, is_system: false, swipe_id: 0, swipes: ['post'] }];
    // Entry saved under an older generation fp → restore is soft-stale.
    _seedSaveCache({
        '2': { '0': { html: '<p>drifted commentary</p>', timestamp: 100, generationFp: 'fp-old' } },
    });
    initCacheRestore({ getGenerationFingerprint: async () => 'fp-new' });

    const pill = makeIndicatorPill();
    const origGetElementById = document.getElementById;
    document.getElementById = id => (id === 'dscIndicator') ? pill
        : (id === 'dscFeed' ? feed : null);
    try {
        const result = await selectCommentaryTarget('2', 0);
        assert.equal(result.status, 'soft-stale');
        assert.ok(pill.classes.has('dsc_stale'), 'indicator carries the stale mark');
        assert.match(pill.title, /regenerate/i, 'tooltip explains the drift');
        assert.ok(pill.classes.has('dsc_pinned'), 'still a pinned (cached) feed');

        // A freshly stored feed is context-fresh — the mark must clear.
        storeFeed('<p>fresh</p>', '2', 0, 'fp-new');
        assert.ok(!pill.classes.has('dsc_stale'), 'fresh store clears the stale mark');
        assert.match(pill.title, /Pinned/);
    } finally {
        document.getElementById = origGetElementById;
    }
});

test('clean-hit restore leaves the indicator without the stale mark', async () => {
    const ctx = getCtx();
    ctx.chat = [null, null, { mes: 'post', is_user: false, is_system: false, swipe_id: 0, swipes: ['post'] }];
    _seedSaveCache({
        '2': { '0': { html: '<p>commentary</p>', timestamp: 100, generationFp: 'fp-current' } },
    });
    ...
    try {
        const result = await selectCommentaryTarget('2', 0);
        assert.equal(result.status, 'hit');
        assert.ok(!pill.classes.has('dsc_stale'), 'no stale mark on a clean hit');
        assert.match(pill.title, /Pinned/);
    } finally {
        document.getElementById = origGetElementById;
    }
